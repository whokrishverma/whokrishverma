...
colored key/value rows for work, tech stack, and highlights.

Static content, hand-authored below in ROWS -- edit that list directly to
update the card. Lines fade/slide in on a short stagger so it feels like the
panel is printing alongside the portrait. STATIC=1 emits the frozen state for
Quick Look previews.

Run: python make_info_card.py
"""
import html
import os
...
TITLEBAR_TEXT = "whokrishverma@github: ~$ infocard"
HOST_USER = "krish"       # green part of the "user@github" header line
HOST_DOMAIN = "github"    # accent part
...
# ("gap",)                     -> vertical space
#
# EDIT THIS LIST to update the card. The Stack section below is currently a
# placeholder copied from a reference card -- swap in your actual stack
# before publishing so it accurately represents your skills.
ROWS = [
    ("host",),
    ("kv", "Now", "AI/ML Intern @ Mankind"),
    ("gap",),
    ("sec", "Stack"),
    ...
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "In development, soon to be deployed"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">{esc(TITLEBAR_TEXT)}</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">{esc(HOST_USER)}</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">{esc(HOST_DOMAIN)}</tspan></text>'
